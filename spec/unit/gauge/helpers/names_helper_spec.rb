...
module Gauge
  module Helpers
    describe NamesHelper do
      ...
      it { expect(NamesHelper).to respond_to :local_name_of, :sql_schema_of, :dbo_key_of }


      describe '.local_name_of' do
        it "extracts the local part of a database object name" do
          ALL_NAMES.each do |dbo_name|
            expect(NamesHelper.local_name_of(dbo_name.downcase)).to eq 'reps'
          end
        end

        it "does not change case of a database object name" do
          expect(NamesHelper.local_name_of('RepProfile')).to eq 'RepProfile'
          expect(NamesHelper.local_name_of(:RepProfile)).to eq 'RepProfile'
        end
      end
      ...
      describe '.sql_schema_of' do
        it "returns 'dbo' as a default SQL schema" do
          NAMES_WITH_DBO_SCHEMA.each do |dbo_name|
            expect(NamesHelper.sql_schema_of(dbo_name)).to eq 'dbo'
          end
        end

        it "extracts SQL schema part for a custom SQL schema" do
          NAMES_WITH_CUSTOM_SCHEMA.each do |dbo_name|
            expect(NamesHelper.sql_schema_of(dbo_name)).to eq 'bnr'
          end
        end

        it "converts SQL schema to lower case" do
          expect(NamesHelper.sql_schema_of('DBO.REPS')).to eq 'dbo'
          expect(NamesHelper.sql_schema_of(:DBO_REPS)).to eq 'dbo'
        end
      end
      ...
      describe '.dbo_key_of' do
        it "returns a database object name with 'dbo' SQL schema converted to a symbol" do
          NAMES_WITH_DBO_SCHEMA.each do |dbo_name|
            expect(NamesHelper.dbo_key_of(dbo_name)).to be :dbo_reps
          end
        end

        it "returns a database object name with custom SQL schema converted to a symbol" do
          NAMES_WITH_CUSTOM_SCHEMA.each do |dbo_name|
            expect(NamesHelper.dbo_key_of(dbo_name)).to be :bnr_reps
          end
        end
      end
