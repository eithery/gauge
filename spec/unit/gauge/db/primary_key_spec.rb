...
module Gauge
  module DB
    describe PrimaryKey do
      let(:key) { PrimaryKey.new('PK_Primary_Reps', :dbo_primary_reps, :rep_code) }
      let(:composite_key) do
        PrimaryKey.new('pk_fund_account_info', :fund_accounts, [:fund_account_number, :CUSIP, 'Carrier_CODE'])
      end
      subject { key }
      ...
      describe '#table' do
        it "equals to the table name passed in the initializer in various forms" do
          ...
          .each do |table_name, actual_table|
            primary_key = PrimaryKey.new('pk_primary_reps', table_name, :rep_code)
      ...
      describe '#clustered?' do
        ...
        context "when specified as nonclustered" do
          before { @nonclustered_key = PrimaryKey.new('pk_reps', :reps, :id, clustered: false) }
          specify { @nonclustered_key.should_not be_clustered }
        end

        context "when specified as clustered" do
          before { @clustered_key = PrimaryKey.new('pk_reps', :reps, :id, clustered: true) }
          specify { @clustered_key.should be_clustered }
        end

        context "when specified with incorrect value" do
          before { @clustered_key = PrimaryKey.new('pk_reps', :reps, :id, clustered: 'no') }
          specify { @clustered_key.should be_clustered }
        end
      end
