...
# A factory class used to build SQL queries.
...
module Gauge
  module SQL
    class Builder
      ...
      def sql_home
        sql_home = ApplicationHelper.sql_home
      ...
      def cleanup(schema)
        schema.cleanup_sql_files(sql_home)
      end
  ...
  private
      ...
      def table_home(table)
        database = create_folder "#{sql_home}/#{table.database}"
      ...
      def file_name_for(command, schema)
        "#{table_home(schema)}/#{prefix(command)}_#{schema.to_sym}.sql"
      end
